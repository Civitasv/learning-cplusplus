...
bool App::OnInitialize() {
  ...
  handler = std::make_unique<SDL_Handler>("Snake", 1280, 720, 300, 200);
  ...
  snake =
      std::make_unique<Snake>(7, 7, size, header_tex, tails_tex, handler.get());

  food = std::make_unique<Food>(size, food_tex, handler.get());
  food->PickLocation(rows, cols);

  // up and bottom wall
  for (int x = 0; x < cols; x++) {
    bricks.push_back(
        std::make_unique<Brick>(x, 0, size, brick_tex, handler.get()));
    bricks.push_back(
        std::make_unique<Brick>(x, rows - 1, size, brick_tex, handler.get()));
  }
  // left and right wall
  for (int y = 0; y < rows; y++) {
    bricks.push_back(
        std::make_unique<Brick>(0, y, size, brick_tex, handler.get()));
    bricks.push_back(
        std::make_unique<Brick>(cols - 1, y, size, brick_tex, handler.get()));
  }
  return true;
}
